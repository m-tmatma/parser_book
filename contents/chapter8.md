...
- **帰納的可算言語（チューリングマシンが認識する言語）:** 最も強力な言語クラスで、チューリングマシンによって受理される言語に対応します。受理されない入力に対してはチューリングマシンが停止するとは限らない（つまり、判定アルゴリズムが存在するとは限らない）点に注意が必要です。判定可能（チューリングマシンが必ず停止する）な部分集合は**帰納的言語**と呼ばれ、こちらが「アルゴリズムで判定可能な言語」に対応します。
