...
ところで、メールアドレスの検証は一見簡単だと思われていますが、実は完全な仕様（RFC 5322）は非常に複雑で、厳密にはコメントの入れ子などを許可するため、正規表現では完全に表現できません。たとえば、以下のような入れ子構造をもったメールアドレスも仕様上は有効です：
...
この規則では、Aはどこに現れても常に`abc`に置き換えることができます。周りの文脈（前後の文字）に関係なく適用できるので「文脈自由」です。自由というと一見「好きにできる」という印象を受けますが、ここの「自由」は、たとえば、アルコールフリー＝アルコールが含まれない、というのと同じ意味での「自由」です。Context-free Grammarを「文脈自由文法」と訳したのはあまりよくないのではないか、という意見もあります。
...
- 文脈依存言語（CSL: Context-Sensitive Language、文脈依存言語）:
  - 文脈依存文法で表現できる**言語の集合**
  - 例：`a^n b^n c^n`（n個のa、n個のb、n個のcがこの順で並ぶ集合）など、文脈自由文法では捉えきれない制約
