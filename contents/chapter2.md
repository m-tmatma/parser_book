...
BNFの観点から言えば、葉ノードは数値リテラルや識別子のような「意味のある値を持つ終端記号」に対応します。一方で、`+`や`-`のような演算子は形式上は終端記号ですが、抽象構文木では演算を表す内部ノードのラベルとして現れる点に注意してください。また、`(`や`)`、`,`のような区切りに使われる終端記号は、一般に抽象構文木では消えてしまい、ノードとしては現れません。
