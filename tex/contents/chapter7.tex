...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{//add ::= term \{"+" term | "{-}" term\}}
\KeywordTok{lazy} \KeywordTok{val}\NormalTok{ add: Parser[AST] = rule\{}
  \FunctionTok{chainl}\NormalTok{(term)(}
\NormalTok{    (\%\% \textless{}\textless{} }\FunctionTok{CL}\NormalTok{(PLUS)) \^{}\^{} \{ location =\textgreater{} (left: AST, right: AST) =\textgreater{} }
        \FunctionTok{BinaryExpression}\NormalTok{(location, Operator.}\FunctionTok{ADD}\NormalTok{, left, right) }
\NormalTok{    \} }
\NormalTok{  | (\%\% \textless{}\textless{} }\FunctionTok{CL}\NormalTok{(MINUS)) \^{}\^{} \{ location =\textgreater{} (left: AST, right: AST) =\textgreater{} }
        \FunctionTok{BinaryExpression}\NormalTok{(location, Operator.}\FunctionTok{SUBTRACT}\NormalTok{, left, right) }
\NormalTok{    \}}
\NormalTok{  )}
\NormalTok{\}}
\end{Highlighting}
...
関数\texttt{CL()}は次のように定義されます。

\begin{Shaded}
\begin{Highlighting}[]
 \KeywordTok{def}\NormalTok{ CL[T](parser: Parser[T]): Parser[T] = parser \textless{}\textless{} SPACING}
\end{Highlighting}
\end{Shaded}

Klassicの構文解析器は自作のパーサーコンビネータライブラリで構築されているので少々ややこしく見えますが、要約すると、\texttt{CL()}は引数に与えたものの後に任意個のスペース（改行）が来るという意味で、キーワードである\texttt{PLUS}や\texttt{MINUS}の後にこのような規則を差し込むことで「式の途中での改行は無視」が実現できています。
...
これを意訳すると、通常の場合はScalaの文はセミコロンまたは改行で終わることができるが、次の三つの条件\textbf{全て}を満たしたときのみ、特別なトークン\texttt{nl}として扱われることになる、ということになります。
